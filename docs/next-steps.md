...
- `agent.py`：precompact 记忆落盘已改为 `asyncio.to_thread(...)`，避免在主事件循环中做同步文件 IO
- `telegram/bot.py`：`/cancel` 已增加显式确认反馈
- `main.py`：历史查询 CLI 已先做 TTL 清理，过期 session 不再被 CLI 误读
- `telegram/html.py`：Telegram 表格渲染已采用代码块方案，并修复 fenced code block 不应被重写的问题
- `weixin/bot.py`：微信最小前端已接通，支持普通对话、进度查询、`/help`、`/new`、`/cancel`、`/plan`、`/sessions`、`/session`
- `session_routes.py`：已支持“当前聊天窗口 -> session_key”路由映射，Telegram / 微信可手动切到同一 session
- `context_builder.py`：新增 `Execution Principles` 注入层，运行时明确要求先确认歧义、最小改动、只改相关代码并用验证收尾
...
### P1.2 Skill 按需注入（已完成基础版）

**来源**：`docs/claude-code-harness-analysis.md` 第 2.2 节

**问题**：原先 `context_builder.py` 会把所有 enabled skill 全文塞进 system prompt。启用 3 个 skill 可能增加 200+ 行 token 开销。

**实现结果**：system prompt 现在只列出 skill 名称、一行摘要和 skill 文件路径；若需要完整说明，agent 通过 `read` 工具按需读取对应 Markdown 文件。

**现状**：已完成基础版，显著降低了 skill 注入的 prompt 膨胀。后续若需要，可继续评估更细的 skill 触发策略。 
...
### P2.4 Function Result Clearing（已完成）

**来源**：`docs/claude-code-harness-analysis.md` 第 2.3 节

Claude Code 只保留最近 N 个工具结果，更早的自动清除。当前 ragtag_crew 已在 compaction 后对较旧的 tool result 做细粒度清理，只保留最近一段结果全文，其余结果截断为带长度标记的占位文本。
...
3. **自动 memory 写入（评估版）**：已完成评估版，但默认关闭。仅对允许来源类型（默认 `search,openapi`）且“非错误结果 + 成功提取引用”的结果尝试去重追加到 `memory/inbox.md`
...
Meta-Harness 附录 D 建议"提供小 CLI 查询历史，方便人工介入和调试"。这部分基础版已经完成，并已补齐 TTL 一致性。

**已完成**：
- `ragtag-crew --history-list`：列出已保存 session
- `ragtag-crew --history <session_key>`：查看该会话的模型、工具预设、session_prompt、protected_content、session_summary 与最近消息
- REPL 终端模式已补齐实时流式输出（ReplStreamer）、执行轨迹收集（TraceCollector）和会话持久化
- `session_store.py` 已从 `telegram/` 提升到包根目录，Telegram、微信与 REPL 共用同一套持久化逻辑
- `session_routes.py` 已提供当前聊天窗口到 session 的 override 路由层
- `prompts.py` 提取了共用的 `DEFAULT_SYSTEM_PROMPT`，三个入口共享系统提示词

**剩余重点**：
- 为 `trace` 增加更直接的查询/筛选入口，而不只是落 JSONL 文件
- 增加最小 CI，至少自动跑 `uv run pytest tests/`
- 继续收口 `/skills`、`/ext`、`/context` 等控制面的契约文档

### M8：上下文系统高级增强

`context-system-design.md` 规划的后续阶段：

1. Protected content 规则（已完成基础版）
2. Block 化 compression state（已完成最小版）
3. Memory search（已完成基础版）
4. `/prompt` 会话级临时规则（已完成）
5. 压缩前记忆落盘策略（已完成评估版）
6. 外部检索层评估（qmd 等）
...
- **Memory search**：已完成基础版，支持 `memory_search` 内置工具与 `/memory search <query>` 命令，在 `MEMORY.md` 和 `memory/*.md` 中做按需检索
- **/prompt**：已完成，支持会话级 `session_prompt` 设置 / 清空 / 查看
- **Protected Content**：已完成基础版，新增独立的 `Protected Content` 注入层，并支持持久化到 session
- **压缩前记忆落盘**：已完成评估版，默认关闭。当前仅在会话压缩前，对带显式记忆标记的旧 user/assistant 消息做保守提取并去重写入 `memory/inbox.md`
- **Compression Blocks**：已完成最小版。每次 compaction 会生成独立 `compression_blocks` 条目并持久化，同时在 system prompt 中单独渲染 `Compression Blocks` 段，现阶段仍与 `session_summary` 并存；后续需要收敛二者分工，避免重复注入
...
| 内容 | 位置 |
|------|------|
| `_OUTPUT_LIMIT = 50_000` + `_truncate()` + `_clip()` | 已提取到 `external/_utils.py` |

该项已完成，后续主要关注类似辅助逻辑是否继续在 `external/`、`tools/` 等目录中重复生长。
...
| 优先级 | 项 | 状态 |
|--------|-----|------|
| P0 | rg 自动下载机制 | `bin_resolver.py` 已有下载/缓存骨架，但 `search_tools.py` 还没完全接到这条链路 |
| P1 | grep/find 只读路径放宽 | 已完成；只读搜索路径已不再受写路径沙箱限制 |
...
```
base → planning → PROJECT.md → USER.local.md → MEMORY.md → skills → policy → protected_content → session_prompt → session_summary → compression_blocks
```

前五段相对静态，后五段每次调用都变。如果后端支持 prefix cache（如 Claude 的 prompt caching），应明确分界标记。另一个现实问题是 `session_summary` 与 `compression_blocks` 当前都在 prompt 中，后续应继续收敛二者分工。取决于 litellm 转发后端是否支持，暂不强制。
...
M6    外部结果进入 summary / memory 的规则收敛
       ↓
M7    trace 查询 + 最小 CI + 控制面补完
```

**现阶段原则**：项目已从“接更多能力”进入“让现有能力更可靠、更可验证、更可治理”的阶段。下一轮不应继续盲目加能力点，而应先把契约、验证和诊断链路做实。
...
- history CLI 已与 session TTL 规则对齐，调试入口更接近真实运行语义
- 相关回归测试已扩充到 280 个，并全部通过

### 下一轮衔接建议

本轮完成后，建议按以下顺序进入下一阶段：

1. 文档与状态收口（README / roadmap / next-steps / skills 契约）
2. P3 两阶段调用默认可靠化
3. P0 搜索工具链工程化闭环（`rg` 自动发现/下载与 `fd`）
4. M6 外部结果进入上下文系统
5. M7 trace 查询入口与最小 CI

原因：现阶段的关键矛盾已经不是“能不能接更多能力”，而是“现有能力是否稳定、可验证、可解释”。P3 的收益最高，但它会显著增加消息量，必须建立在摘要质量、文档契约和诊断能力继续收敛的前提上。
