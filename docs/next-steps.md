...
### P3 两阶段调用（draft + verify，已完成基础版）

**来源**：Meta-Harness 论文 + `project-roadmap.md` M7 待做项

**方案**：先生成变更，再独立验证（lint、类型检查、测试），确认后算完成。

**实现结果**：当前在本轮检测到文件修改后，可按配置自动注入验证指令，再跑有限轮 verify loop。默认关闭，需要显式开启 `verify_enabled`。
...
- **Compression Blocks**：已完成最小版。每次 compaction 会生成独立 `compression_blocks` 条目并持久化，同时在 system prompt 中单独渲染 `Compression Blocks` 段，现阶段仍与 `session_summary` 并存
...
```
base → planning → PROJECT.md → USER.local.md → MEMORY.md → skills → policy → protected_content → session_prompt → session_summary
```
