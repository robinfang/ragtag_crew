...
| M4 | MCP client | ✅ |
| M5 | 固定 OpenAPI 工具 | ✅ |
| M7（部分） | 执行轨迹收集、/cancel、/model、dev 模式、Planning 机制、运行时进度追踪、env bootstrap、表格渲染、历史查询 CLI、两阶段调用基础版 | ✅ |
...
| 提交 | 内容 |
|------|------|
| `abec3a4` | 修复 precompact 记忆落盘阻塞事件循环；history CLI 补 TTL 一致性 |
...
- `session_store.py` 已从 `telegram/` 提升到包根目录，Telegram 与 REPL 共用同一套持久化逻辑
- `prompts.py` 提取了共用的 `DEFAULT_SYSTEM_PROMPT`，两个入口共享系统提示词
